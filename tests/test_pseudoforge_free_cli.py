...
class PseudoForgeFreeCliTests(unittest.TestCase):
    ...
    def test_free_cli_writes_ida_free_artifacts_without_ida_modules(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("profile_root", payload["results"][0])
            self.assertIn("active_profiles", payload["results"][0])
            self.assertIsInstance(payload["results"][0]["active_profiles"], list)
            ...
            self.assertIn("profile_root", summary)
            self.assertIn("active_profiles", summary)
